...
const VerifyOTP = ({ email, onVerified }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${config.API_BASE_URL}/api/auth/verify-otp`, { email, otp });
      setMessage(res.data.message);
      onVerified(otp);  // Pass OTP to parent component
    } catch (error) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
        ...
        <form onSubmit={handleSubmit} className="mt-4">
          ...
          {message && <p className={`text-sm ${message.includes("Invalid") ? "text-red-500" : "text-green-500"}`}>{message}</p>}
